	...
	<div class="panel panel-default">
		...
		@foreach($categories as $category)
			<div class="row">
				...
				@if($category->post->ratings->where('useful', 1)->count() >= $category->post->ratings->where('useful', 0)->count())
			  					{{ $category->post->ratings->where('useful', 1)->count() }}
			  				@else
			  					-{{ $category->post->ratings->where('useful', 0)->count() }}
